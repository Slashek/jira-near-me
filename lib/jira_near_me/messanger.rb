...
module JiraNearMe
  class Messanger
    ...
    def print_release_info(projects, commits, options)
      log(:print_commit_info,
          commits_log: commits.map { |commit| commit }.join("\n"))
      print_pre_release_message(projects)
      print_user_confirmation unless options.key?(:skip_confirmation)
    end
    ...
    def slack(message_name)
      slack_notifier.ping(messages[message_name], icon_emoji: ':see_no_evil:')
    end
